            ...
            .offset(x: -300, y: -200)
                ...
                Button(action: {
                    ...
                    ZStack {
                        ...
                        VStack {
                            Text(String(format: "%02d:%.02d:%02d", Int(round(model.hours_left_double)), Int(round(model.minutes_left_double)), Int(round(model.seconds_left_double))))
                                .foregroundColor(.white)
                                .fontWeight(.bold)
...
// MARK: Minutes Slider Circle
            ...
            Group {
                ...
                Circle()
                    .trim(from: 0.0, to:  model.minutes_left_double / model.minutes_total_double)
                ...
                Circle()
                    ...
                    .gesture(DragGesture(minimumDistance: 0.0)
                                .onChanged({ value in
                                    model.minutes_slider_change(location: value.location)
                                }))
...
// MARK: Hours Slider Circle
            Group {
                ...
                Circle()
                    .trim(from: 0.0, to:  model.hours_left_double / model.hours_total_double)
                    .stroke(Color.white, lineWidth: 5.0)
                    .foregroundColor(Color(red: 0, green: 0.1, blue: 0.3, opacity: 0.5))
                    .frame(width: model.hours_circle_radius * 2, height: model.hours_circle_radius * 2, alignment: .center)
                    .rotationEffect(.degrees(-90))
                ...
                Circle()
                    ...
                    .gesture(DragGesture(minimumDistance: 0.0)
                                .onChanged({ value in
                                    model.hours_slider_change(location: value.location)
                                }))
            }
...
// MARK: Seconds Slider Circle
            ...
            Group {
                ...
                Circle()
                    .trim(from: 0.0, to:  model.seconds_left_double / model.seconds_total_double)
                    .stroke(Color.white, lineWidth: 5.0)
                    .foregroundColor(Color(red: 0, green: 0.1, blue: 0.3, opacity: 0.5))
                    .frame(width: model.seconds_circle_radius * 2, height: model.seconds_circle_radius * 2, alignment: .center)
                    .rotationEffect(.degrees(-90))
                ...
                Circle()
                    ...
                    .gesture(DragGesture(minimumDistance: 0.0)
                                .onChanged({ value in
                                    model.seconds_slider_change(location: value.location)
                                }))
